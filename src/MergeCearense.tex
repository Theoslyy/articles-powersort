...
% Author info
\title{\textbf{A study in optimality: MergeCearense}}
\author{Victor Almeida Campos \and Théo Araújo Magalhães \and Rafael Sobrenome UltimoNome} %Existe ordem própria ou é só ordem alfabética?
% \author{
% 	Victor Campos
% 	% \and
% 	% Raul Lopes
...
version: MergeCearense. We demonstrate (hopefully) that our method is competitive
and that our trie-tree view helps explain and better visualize optimal merge run strategies.
\end{abstract}

\section{Introduction}
Sorry I ain't writing this now. 
\subsection{Outline}
tem um nome melhor pra essa seção, a ideia é servir um sumário pra cada seção do artigo. esqueci o nome.

\section{Context}

\section{The Trie-Trees}

\section{Powersort and optimality}
nas nossas issues, tem primeiro falar da cartesian tree e depois do powersort.
