...
\author{Victor Almeida Campos \and Théo Araújo Magalhães \and Rafael Sobrenome UltimoNome \and Pedro Henrique Sobrenome últimoNome} 
...
\begin{abstract}
We propose a novel way of visualizing natural mergesort algorithms: as a trie-tree of 
indexes. The utility of our framework of interpretation will be shown through a reinterpretation of
a state of the art sorting algorithm: Powersort. We also demonstrate an\footnote{(Hopefully)}
improved version of Powersort: MergeCearense and show that it is competitive.
\end{abstract}

\section{Introduction - ESBOÇO}
There is a variety of different ways to understand and analyze sorting algorithms. Traditionally, the
study of a sorting algorithm's optimality fell under scrutiny of the number of comparisons an algorithm makes, which 
resulted in a lower bound for sorting of $\mathcal{O}(N \log N)$ comparisons\footnote{ref here, woof}. 
This lower bound, though, doesn't mean the study of sorting is over and that we can't  analyze other strategies: the study 
of external sorting algorithms has resulted in a different sorting lower bound of $\mathcal{O}({N \over B} \log_{M \over B} {N \over B})$ \footnote{ref 2 here, woof!},
and adaptive algorithms require specific analyzes to determine optimality in regards of which measure an algorithm is 
trying adapting to. Currently, it is of our opinion that the most interesting results in sorting have come out of 
the study of natural mergesort algorithms. These algorithms try to adapt to the entropy of runs and are the object of study
of this article. 

\noindent\textbf{Theo:} o segmento comentado (ver código tex) abaixo era eu tentando pensar em uma forma de falar como
é legal estudar algoritmos que estão relacionados com a estrutura do problema,
mas que isso é difícil e se feito de forma ruim pode ser muito ruim e 
dizer que é mais ou menos isso que está acontecendo com o estudo de algoritmos cache oblivious
e que, de forma contrária, não é o que está acontecendo com algoritmos de mergesort natural. 
\newline \noindent \textbf{end of comment}

% To understand how the study of sorting algorithms is progressing is also to understand, somewhat, how science progresses: 
% classic sorting algorithms (i.e Quicksort, Mergesort, Heapsort) have simple worst-case proofs, whilst Timsort,
% on the other hand, was implemented in Python two decades before a theoretical bound for it was proven. The classic
% algorithms do not meddle with the underlying structure of the problem, whilst Timsort was the first good (in practice)
% attempt of exploiting sorted segments in an array. To meddle with the structure of a problem and use this structure as a way to solve the problem
% requires, sometimes, more effort than simply just ignoring any underlying structure. Cache oblivious algorithms seem to, sadly, 
% be 
...
\noindent The study of sorting algorithms has become a rather complex and dense endeavor, with
...
We aim, within this and further sections, to define all of the concepts necessary to understand this paper in a self contained way, as to avoid
...
\begin{tikzpicture}[level distance=\lvld cm,
  ...
  %   BLUE TREE (drawn first to let the middle line filled in pink)

  \node[gblock=8,yshift=\shft cm] (A') {3 \nodepart{two} 9 \nodepart{three} 10 \nodepart{four} 27 \nodepart{five} 39 \nodepart{six} 43 \nodepart{seven} 56 \nodepart{eight} 82}
      [grow=up,edgeup]
      child {node[gblock=4] (B2') {9 \nodepart{two} 10 \nodepart{three} 56 \nodepart{four} 82}
          child {node[gblock=2] (C4') {10 \nodepart{two} 56}
              child {node[gblock=1] (D8') {56}}
  ...
  \node[block=8] (A) {39 \nodepart{two} 27 \nodepart{three} 43 \nodepart{four} 3 \nodepart{five} 9 \nodepart{six}82 \nodepart{seven}10 \nodepart{eight} 56}
      ...
      child {node[block=4] (B2) {9 \nodepart{two} 82 \nodepart{three} 10 \nodepart{four} 56}
          child {node[block=2] (C3) {9 \nodepart{two} 82}
              child {node[block=1] (D5) {9}}
              child {node[block=1] (D6) {82}}
          }
          child {node[block=2] (C4) {10 \nodepart{two} 56}
              child {node[block=1] (D7) {10}}
              child {node[block=1] (D8) {56}}
          }
      };
\end{tikzpicture}
\caption{Mergesort tree with 8 elements}
\end{center}
\end{figure}

\noindent The figure above shows us how the algorithm functions: it divides the array in half and
recursively descends in each halved part until the resulting split creates subproblems of size one. Once
it is in a subarray of size one, it merges this array with its neighbor, which is also of size one, starting a chain of merges.
This chain can be seen in the image right after the last pink blocks of size one: first, the eight individual pink blocks are merged together 
into subarrays of size 2, then, the now-blue and 2-sized subarrays merge, resulting in two subarrays of size four, which then
leads to the merge of the two remaining subarrays of size four, creating the final sorted array of size 8.
In a generic way, the Mergesort algorithm functions by dividing the main array into two subarrays, i.e into two sorting subproblems, solving these subproblems recursively
and merging the results of both subproblems, resulting into the final ordered array.
If you look closely at the MergeSort tree above, though, you can see that the subproblem
...
our Mergesort explanation above, it can be seen a subproblem that is already solved.

\noindent \newline After defining runs, it is natural to think about engineering a MergeSort algorithm 
...
\noindent \newline But what's the advantage of a Natural Mergesort over traditional Mergesort? The idea here is that
Natural Mergesort adapts to the structure of the array, thus, instead of recursively descending 
until a subproblem of size one, resulting in a tree of logarithmic size in relation to the input, Natural Mergesort
'knows' more about the array, thus, it 'tries' to create a smaller tree. 

\definition{} A \textbf{MergeTree} is a tree that gives us merges, duh. (to-do)

\noindent \newline But how good can the Natural Mergesort tree be? What does it adapt to? 

\definition{} \textbf{Entropy} is a measure of the RUNS present in the ARRAY (to-do definition)

\noindent \newline But how does a tree that adapts to entropy looks like?

\subsection{Merge Trees}
\textbf{Theo:} After defining Entropy, do we spend some time talking about Huffman
and other sorting algorithms? Do we jump straight to how powersort does it?
I think that talking about Huffman and how the huffman tree was used for 
a mergesort algorithm is interesting. 
\newline \textbf{end of comment}
\definition{} \textbf{Huffman Encoding}
\definition{} \textbf{Binary Search Tree}
...
datastructure that will serve as the basis for the way we visually interpret natural mergesort algorithms. 
