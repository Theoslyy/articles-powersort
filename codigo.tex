...
\While{$true$}
    \While{$e_1 < n$} 
        ...
        \While {X.size \textgreater \text{ }1 and X.top().power $>$ p}
            \State s0 $\gets$ X.pop().boundary
            \State Merge(A, s0, s1, e1)
            \State s1 $\gets$ s0
    ...
    \While{$X.size > 1$}
        ...
        \State s1 $\gets$ s0
    \EndWhile
    \State s1 $\gets$ X.pop().boundary
    \State swap(s1, e1) \Comment{Simplification for s1 gets e1 and e1 gets s1. This signals the start of the scan left.}
    \State \Comment{After merging everything that was on the stack, we will be left with a
    run at the end of the array. We will use this run as our first run in our scan left. This
    saves on doing an initial scan left to 'switch sides' and find the first run.}
    \If{$e_1 == 1$}
    ...
    \EndWhile
    \State s1 $\gets$ X.pop().boundary
    \State swap(s1, e1) \Comment{Switching to the scan right. The equivalent of what we did before scanning left.}
    \If{$e_1 == n$}
    \State break
    \EndIf
\EndWhile
...
\State $\ell$ := \_{\_{}}builtin\_{}clz(xor)
\State \textbf{return ($\ell$)}
\end{algorithmic}
\vspace{-1em}
\noindent\rule{\linewidth}{0.5pt}\par
