...
\section*{Abstract:}
This paper presents a variation of Powersort, a stable mergesort algorithm. 
The original implementation of Powersort utilizes a $\log n + 1$-sized stack of existing runs and 
sorts an array in a singular pass\cite{https://doi.org/10.4230/lipics.esa.2018.63}.
Our algorithm works by doing multiple passes over the array, keeping a limited stack of runs. 
We call this algorithm "C-Bounded Powersort" (placeholder). Moreover,
we present an alternative and faster Node power algorithm and
show that it gives the same guarantees as the one present in Munro $\&{}$ Wild's Powersort.
\section{Introduction:} Sorting is a widely talked about problem in computer science, with a 
variety of novel ideas being tested in search of more efficient algorithms. Currently,
a rather active field of research is that of sorting algorithms that adapt to their input. In our case,
we are concerned with mergesort algorithms that adapt to the presence of exisiting sorted segments in the
array, which is what we call a run. Mergesort algorithms that adapt to the
presence of runs are often called "natural mergesorts"\cite{https://doi.org/10.4230/lipics.esa.2018.63}.
For further context and somewhat of a historiographical review, please check Munro $\&{}$ Wild's papers
on Powersort and Multiway Powersort.\cite{Gelling_2023}\cite{https://doi.org/10.4230/lipics.esa.2018.63}
\subsection{Contributions:} 
We kind of need to do the experiments and run the stuff to talk about our contributions :)
So this is a TO-DO!
\subsection{Related works:}
TO-DO!
% Any more section sin the introduction?
\section{Definitions:}
Here, we define useful terms and ideas that will be frequently
cited in our proofs section. Some of the definitions 
are not ours, some we tweaked to fit our purposes, and others 
we defined for better organization of the paper. 
\subsection{General definitions:}
These definitions are related to generic natural mergesorts. 

\textbf{Runs:} Contiguous regions of the input that are already in sorted order. 
Let R$_0$, \dots , R$_{r-1}$ be the runs in the input, L$_i$ denotes the length of R$_i$.\cite{https://doi.org/10.4230/lipics.esa.2018.63}

\textbf{Run boundaries:} Denoted by B$_i$, it is the boundary between the (i − 1)st and ith run (i=1, \dots, r - 1).\cite{Gelling_2023}

\textbf{Node depth:} d$_i$ is the depth of leaf R$_i$ in T, where depth is the number of edges on the path to the root.\cite{Gelling_2023}

\textbf{Run-adaptive mergesort and merge tree:} Any stable run-adaptive mergesort can be described as a merge tree T: 
...
\cite{Gelling_2023}\cite{https://doi.org/10.4230/lipics.esa.2018.63}

\textbf{Merge costs:} M(v) is the total length of the resulting merge corresponding to the internal node v.
The total merge cost of merge tree T is then: $M(T) = \sum_{v \in T}M(v)$. 
Given that each node can be atributed to the merge between two runs, we can also calculate
at the merge costs by analyzing how many times elements are counted:
every element in run R$_i$ is counted exactly d$_i$ times in $\sum$$_v$M(v), so: 
\begin{equation}
        M(T) = \sum^{r-1}_{i=0}d_i \cdot L_i*    
\end{equation}
\cite{Gelling_2023}\cite{https://doi.org/10.4230/lipics.esa.2018.63}
\subsection{Powersort definitions:}
These definitions are related to the Powersort algorithm.
...
(Pj is the index of the first bit where the (binary) fractional parts of a and b differ.) \cite{https://doi.org/10.4230/lipics.esa.2018.63}

\textbf{Powersort:} Powersort is a mergesort algorithm that decides when two runs are ready to be
merged through the following merge policy: by simulating a nearly optimal binary search tree through Mehlhorn's 
second method\cite{m75}, the algorithm compares the node power p between runs k and k+1 with the node power between
runs k-1 and k, p'. If p' > p, the algorithm merges run k with run k'. The algorithm keeps a stack of 
runs and a stack of powers, therefore, when a merge happens, it pops both stacks and keeps on comparing the node power on top of the stack with p, 
until p' > p is false. Once the algorithm reaches the end of the array, it simply merges all runs left in the stack, which
guarantees the entire array will be sorted. 

\textbf{Forgetful K-Stack/C-Bounded Stack:} A forgetful k-stack is a stack that 'forgets' its first element if it already has k elements
in it and the (k+1)-ith is added. This is simply a circular stack in which we overwrite bottom elements when the stack is full. 

\textbf{C-Bounded Powersort:} A variation of the Powersort algorithm that uses a Forgetful K-Stack instead of the stack with 
size $\log n + 1$. 
...
\section{Proofs:} 
Here, we prove that our algorithm is optimal and has the same asymptotic complexity as that of the original Powersort.

\textbf{Proposition:} Let T be a Powersort merge tree of height H. One pass of C-Bounded Powersort is 
...
\begin{tikzpicture} [
    ...
    minimum width=0.60cm, minimum height=0.90cm,
    align=center}},
...
R$_1$  will be scanned and, then, the algorithm will search for the adjacent run, R$_2$. After scanning R$_2$, 
the algorithm will calculate the power between R$_1$ and R$_2$, which is equal to the height of v$_1$. Given that 
the stack of powers is empty, there will be no merges and R$_1$ will be added to the stack. Afterwards, the algorithm will proceed to the
...
our previously calculated power, leading to the merge of the run on top of the stack, R$_1$ with our current run R$_2$,
creating run R$_{12}$. Then, the stack of powers and of runs will be empty and run R$_{12}$ will be added to the stack
of runs whilst the power between R$_2$ and R$_3$ will be added to the stack of powers.

After this, our Forgetful 2-Stack will simply contain R$_{12}$. Then, the algorithm will find run R$_4$. 
...
runs and will add R$_3$  to the Forgetful 2-Stack. 
...
still is in the Forgetful-Stack will be merged with our current run, thus, in this situation, a merge between R$_{4}$ and R$_3$ will happen,
creating run R$_{34}$ and both stacks will be popped. Then, a merge between R$_{12}$ and R$_{34}$ will happen.
...
exactly k spaces left, with one space being occupied by the first run of T$_r$, 
...
also to the processing of T', given that R$_\ell$ will still be on the stack alongside all runs from T$_r$.\footnote{
In reality, the last run of T$_r$ will not be in the stack: it will be the the 'current run' in the algorithm whose node power with the 
consecutive run will result in v-1. Merges happen between the current run and the run on top of the stack.
}
...
no começo do artigo e precisam ser revisados para fazerem
sentido, alguns ainda estão bem abstratos e/ou não descrevem corretamente os algoritmos. \newline
\noindent 2. Formatação geral dos algoritmos para que eles fiquem bem posicionados nas páginas, além de
deixar eles mais 'bonitos', não sei qual o formato de apresentação que o professor prefere. \newline 
\noindent \textbf{Introduction:} In this section, we introduce the original pseudocodes for the Powersort and Node Power algorithms
as well as our modified versions.
\subsection{Powersorts:}
...
\subsection{Node Power versions:}
\subsubsection{Standard Node power:}
...
\begin{algorithm2e}
\caption{NodePower($s_1, s_2$)}\label{np2}
...
this is probably due to the fact that they simply use a standard stack of height $\log n + 1$.
Thus, we will have no 'counterpart' algorithm to analyze. Our algorithm implements a modified version of a stack which
we call:
\begin{equation*}
    \text{C-Bounded Stack/Forgetful K-Stack.}
\end{equation*}
...
% \SetKwData{S}{S}
% \SetKwData{top}{S.top}
% \SetKwData{size}{S.size}
% \SetKwData{MaxSize}{S.MaxSize}

$top \gets 0$\; $size \gets 0$\Comment*{Starting values for the stack}
...
\Def{pop$(S)$}{
    ...
    $S.size \gets S.size -1$\;
    $r \gets S.top$\;
    \If{$S.top = 0$}{
        $S.top \gets MaxSize-1$;
    }
    \Else{
        $S.top \gets S.top - 1$\;
    }
...
\Def{push$(S, b, p)$}{ 
    \nlset{}\Comment{$b$ is the boundary of the latest run}
    \nlset{}\Comment{$p$ is the node power between this and the following run.}
    $S.top \gets S.top +1$\;
    \If{S.top > MaxSize}{
        $S.top \gets 0$
    }
    ...
    $S.array[S.top].b \gets b$\;
    $S.array[S.top].p \gets p$\;
 }
\Def{TopPower$(S)$}{
    \Return $S.array[S.top].p$
}
\end{algorithm2e}
\newpage
